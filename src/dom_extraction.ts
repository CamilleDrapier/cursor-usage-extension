...
export const extractRequestCounts = (statsContainer: Element): RequestCounts => {
  ...
  const used = Number(usedText.replace(/\D/g, '')) || 0
  const allowed = Number(allowedText.replace(/\D/g, '')) || 0

  return { used, allowed }
}

const parseTooltipDate = (tooltipContent: string): Date => {
  const parsed = new Date(tooltipContent)
  if (isNaN(parsed.getTime())) throw new Error(`Invalid date format in tooltip: ${tooltipContent}`)
...
export const extractAllowanceEndDate = (doc: Document = document): Date => {
  ...
  return parseTooltipDate(tooltipContent)
}
